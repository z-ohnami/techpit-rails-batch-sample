...
RSpec.describe Chapter5::RankOrderMaker, type: :model do
  describe '#call' do
    let(:score1) { 1 }
    let(:score2) { 2 }
    let(:score3) { 3 }
    ...
    context 'scoreが1件ずつ、異なる場合' do
      before do
        create(:monthly_rank, rank: 0, score: score1)
        create(:monthly_rank, rank: 0, score: score2)
        create(:monthly_rank, rank: 0, score: score3)
      end
